...
function userRequestHandler(req, userRes) {
  ...
  */
  ...
  var notRecord = false// !!ua.match("webview") || ua === navigator.userAgent
  if (!notRecord && GLOBAL.recorder) {
...
function connectReqHandler(req, socket, head) {
  var host = req.url.split(":")[0],
    targetPort = parseInt(req.url.split(":")[1]),
    resourceInfo,
    resourceInfoId;
  ...
  if (targetPort == GLOBAL.option.socketPort) {
  ...
  async.series([
        ...
        if (internalHttpsPort) {
          ...
          getPort(function(port) {
            ...
            if(targetPort === 443) {
              ...
              var httpServer = http.createServer(userRequestHandler)
              httpServer.listen(internalHttpsPort)
    ...
    function(callback) {
      ...
      if (shouldIntercept) {
        ...
        proxyPort = targetPort //(targetPort == 80) ? 443 : targetPort;
        proxyHost = host;
