...
// var appConfig = require(global.main_path + 'browser/config/appConfig.js')
...
function Recorder(option){
    ...
    self.appendRecord = function(info){
        ...
        // todo
        // if(info.req && info.req.headers && info.req.headers['user-agent'].match(global.__appname))
        //     return

        var url = info.url
        if(url.match('/ws/target'))
            return
        if(url.match('/ws/client/'))
            return
        if(url.match(/localhost/i))
            return
        if(url.match(/client\/weinre\//))
            return

        try{
            var ua = info.req.headers['user-agent']
            var find = ua.indexOf('wechatdevtools')
            if(find !== -1)
                return
            else if(ua === navigator.userAgent)
                return
        } catch(e){}
