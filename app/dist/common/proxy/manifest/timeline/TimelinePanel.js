...
{this.setTiles([{layerId:layer.id(),rect:[0,0,layer.width(),layer.height()],imageURL:imageURL}])},setTiles:function(tiles)
{this._picturesForLayer={};tiles.forEach(this._setTile,this);},_setTile:function(tile)
{var texture=this._gl.createTexture();texture.image=new Image();texture.image.addEventListener("load",this._handleLoadedTexture.bind(this,texture,tile.layerId,tile.rect),false);texture.image.src=tile.imageURL;},_initGL:function(canvas)
...
borderColor=WebInspector.Layers3DView.BorderColor;var borderWidth=isSelected?WebInspector.Layers3DView.SelectedBorderWidth:WebInspector.Layers3DView.BorderWidth;return{color:color,borderColor:borderColor,borderWidth:borderWidth};},_calculateVerticesForQuad:function(quad,z)
...
{var gl=this._gl;var vertices;var style=this._styleForLayer(layer);var layerDepth=this._depthByLayerId[layer.id()]*WebInspector.Layers3DView.LayerSpacing;if(this._isVisible[layer.id()]){vertices=this._calculateVerticesForQuad(layer.quad(),layerDepth);gl.lineWidth(style.borderWidth);this._drawRectangle(vertices,style.borderColor,gl.LINE_LOOP);gl.lineWidth(1);}
this._scrollRectQuadsForLayer[layer.id()]=this._calculateScrollRectQuadsForLayer(layer);var scrollRectQuads=this._scrollRectQuadsForLayer[layer.id()];for(var i=0;i<scrollRectQuads.length;++i){vertices=this._calculateVerticesForQuad(scrollRectQuads[i],this._calculateScrollRectDepth(layer,i));var isSelected=this._isObjectActive(WebInspector.Layers3DView.OutlineType.Selected,layer,i);var color=isSelected?WebInspector.Layers3DView.SelectedScrollRectBackgroundColor:WebInspector.Layers3DView.ScrollRectBackgroundColor;this._drawRectangle(vertices,color,gl.TRIANGLE_FAN);this._drawRectangle(vertices,WebInspector.Layers3DView.ScrollRectBorderColor,gl.LINE_LOOP);}
var tiles=this._picturesForLayer[layer.id()]||[];for(var i=0;i<tiles.length;++i){var tile=tiles[i];var quad=this._calculateRectQuad(layer,{x:tile.rect[0],y:tile.rect[1],width:tile.rect[2]-tile.rect[0],height:tile.rect[3]-tile.rect[1]});vertices=this._calculateVerticesForQuad(quad,layerDepth);this._drawRectangle(vertices,style.color,gl.TRIANGLE_FAN,tile.texture);}},_drawViewport:function()
{var viewport=this._layerTree.viewportSize();var vertices=[0,0,0,viewport.width,0,0,viewport.width,viewport.height,0,0,viewport.height,0];var color=[0,0,0,1];this._gl.lineWidth(3.0);this._drawRectangle(vertices,color,this._gl.LINE_LOOP);this._gl.lineWidth(1.0);},_calculateDepths:function()
...
this._emptyView.detach();var gl=this._initGLIfNecessary();this._resizeCanvas();this._initProjectionMatrix();this._calculateDepths();gl.viewport(0,0,gl.viewportWidth,gl.viewportHeight);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);if(this._layerTree.viewportSize())
this._drawViewport();this._layerTree.forEachLayer(this._drawLayer.bind(this));},_intersectLineAndRect:function(vertices,matrix,x0,y0)
...
return undefined;}
return t;},_activeObjectFromEventPoint:function(event)
{if(!this._layerTree)
...
{var activeObject=this._activeObjectFromEventPoint(event);var node=activeObject&&activeObject.layer&&activeObject.layer.nodeForSelfOrAncestor();var contextMenu=new WebInspector.ContextMenu(event);contextMenu.appendItem("Reset view",this._transformController._resetAndNotify.bind(this._transformController),false);if(node)
...
{var object=this._activeObjectFromEventPoint(event);if(object&&object.layer)
this.dispatchEventToListeners(WebInspector.Layers3DView.Events.LayerSnapshotRequested,object.layer);event.stopPropagation();},__proto__:WebInspector.VBox.prototype};WebInspector.MemoryCountersGraph=function(delegate,model,uiUtils)
...
this.addRecord(records[i]);},__proto__:WebInspector.CountersGraph.prototype};WebInspector.TimelineModel=function(target)
{WebInspector.TargetAwareObject.call(this,target);this._filters=[];}
...
{return false;},forAllRecords:function(preOrderCallback,postOrderCallback)
...
{throw new Error("Not implemented");},loadFromURL:function(url,progress)
{throw new Error("Not implemented");},saveToFile:function()
{throw new Error("Not implemented");},reset:function()
{this._loadedFromFile=false;this._records=[];this._minimumRecordTime=0;this._maximumRecordTime=0;this._mainThreadTasks=[];this._gpuThreadTasks=[];this._eventDividerRecords=[];this.dispatchEventToListeners(WebInspector.TimelineModel.Events.RecordsCleared);},minimumRecordTime:function()
...
var result=this._backgroundRecordsBuffer.mergeOrdered(records,recordTimestampComparator);this._backgroundRecordsBuffer=[];return result;}};WebInspector.TimelineModelImpl=function(timelineManager)
{WebInspector.TimelineModel.call(this,timelineManager.target());this._timelineManager=timelineManager;this._filters=[];this._bindings=new WebInspector.TimelineModelImpl.InterRecordBindings();this.reset();this._timelineManager.addEventListener(WebInspector.TimelineManager.EventTypes.TimelineEventRecorded,this._onRecordAdded,this);this._timelineManager.addEventListener(WebInspector.TimelineManager.EventTypes.TimelineStarted,this._onStarted,this);this._timelineManager.addEventListener(WebInspector.TimelineManager.EventTypes.TimelineStopped,this._onStopped,this);this._timelineManager.addEventListener(WebInspector.TimelineManager.EventTypes.TimelineProgress,this._onProgress,this);}
WebInspector.TimelineModelImpl.TransferChunkLengthBytes=5000000;WebInspector.TimelineModelImpl.prototype={loadedFromFile:function()
{return this._loadedFromFile;},startRecording:function(captureStacks,captureMemory,capturePictures)
{console.assert(!capturePictures,"Legacy timeline does not support capturing pictures");this._clientInitiatedRecording=true;this.reset();var maxStackFrames=captureStacks?30:0;var includeGPUEvents=WebInspector.experimentsSettings.gpuTimeline.isEnabled();var liveEvents=[WebInspector.TimelineModel.RecordType.BeginFrame,WebInspector.TimelineModel.RecordType.DrawFrame,WebInspector.TimelineModel.RecordType.RequestMainThreadFrame,WebInspector.TimelineModel.RecordType.ActivateLayerTree];this._timelineManager.start(maxStackFrames,WebInspector.experimentsSettings.timelineNoLiveUpdate.isEnabled(),liveEvents.join(","),captureMemory,includeGPUEvents,this._fireRecordingStarted.bind(this));},stopRecording:function()
...
this._clientInitiatedRecording=false;this._timelineManager.stop(this._fireRecordingStopped.bind(this));},records:function()
...
this.reset();if(event.data){this._fireRecordingStopped(null,null);}},_onProgress:function(event)
...
{this._collectionEnabled=false;if(cpuProfile)
...
{this._loadedFromFile=false;this._payloads=[];this._stringPool={};this._bindings._reset();WebInspector.TimelineModel.prototype.reset.call(this);},_internStrings:function(record)
...
{return WebInspector.TimelineUIUtils.recordStyle(this).category;},startTime:function()
...
timelineModel.forAllRecords(processRecord);};WebInspector.TimelineOverviewPane=function(model,uiUtils)
{WebInspector.VBox.call(this);this._uiUtils=uiUtils;this.element.id="timeline-overview-pane";this._eventDividers=[];this._model=model;this._overviewGrid=new WebInspector.OverviewGrid("timeline");this.element.appendChild(this._overviewGrid.element);this._overviewCalculator=new WebInspector.TimelineOverviewCalculator();model.addEventListener(WebInspector.TimelineModel.Events.RecordsCleared,this._reset,this);this._overviewGrid.addEventListener(WebInspector.OverviewGrid.Events.WindowChanged,this._onWindowChanged,this);this._overviewControls=[];}
...
continue;var divider=this._uiUtils.createEventDivider(record.type());divider.style.left=positions.start+"%";dividers[dividerPosition]=divider;}
this._overviewGrid.addEventDividers(dividers);},addRecord:function(record)
{var eventDividers=this._eventDividers;var uiUtils=this._uiUtils;function addEventDividers(record)
...
parentRecord=coalescedRecord;var formattedRecord=new WebInspector.TimelinePresentationModel.ActualRecord(record,parentRecord);this._recordToPresentationRecord.put(record,formattedRecord);formattedRecord._collapsed=parentRecord===this._rootRecord;if(coalescingBucket)
...
{var parentRecord=presentationRecord._presentationParent;WebInspector.TimelineUIUtils.aggregateTimeByCategory(parentRecord.presentationAggregatedStats(),presentationRecord.presentationAggregatedStats());if(parentRecord.endTime()<presentationRecord.endTime())
...
{throw new Error("Not implemented.");},presentationAggregatedStats:function()
...
{return this._record;},presentationAggregatedStats:function()
{return this._record.aggregatedStats();},hasWarnings:function()
...
{WebInspector.TimelinePresentationModel.Record.call(this,null);this._startTime=record.startTime();this._endTime=record.endTime();this._aggregatedStats={};}
...
{return this._presentationChildren[0].record();},presentationAggregatedStats:function()
{return this._aggregatedStats;},coalesced:function()
...
{WebInspector.TimelinePresentationModel.Record.call(this,null);this._aggregatedStats={};}
WebInspector.TimelinePresentationModel.RootRecord.prototype={presentationAggregatedStats:function()
{return this._aggregatedStats;},hasWarnings:function()
{return false;},__proto__:WebInspector.TimelinePresentationModel.Record.prototype};WebInspector.TracingTimelineModel=function(tracingModel)
{WebInspector.TimelineModel.call(this,tracingModel.target());this._tracingModel=tracingModel;this._mainThreadEvents=[];this._inspectedTargetEvents=[];this.reset();}
WebInspector.TracingTimelineModel.RecordType={Program:"Program",EventDispatch:"EventDispatch",GPUTask:"GPUTask",RequestMainThreadFrame:"RequestMainThreadFrame",BeginFrame:"BeginFrame",BeginMainThreadFrame:"BeginMainThreadFrame",ActivateLayerTree:"ActivateLayerTree",DrawFrame:"DrawFrame",ScheduleStyleRecalculation:"ScheduleStyleRecalculation",RecalculateStyles:"RecalculateStyles",InvalidateLayout:"InvalidateLayout",Layout:"Layout",UpdateLayer:"UpdateLayer",PaintSetup:"PaintSetup",Paint:"Paint",PaintImage:"PaintImage",Rasterize:"Rasterize",RasterTask:"RasterTask",ScrollLayer:"ScrollLayer",CompositeLayers:"CompositeLayers",ParseHTML:"ParseHTML",TimerInstall:"TimerInstall",TimerRemove:"TimerRemove",TimerFire:"TimerFire",XHRReadyStateChange:"XHRReadyStateChange",XHRLoad:"XHRLoad",EvaluateScript:"EvaluateScript",MarkLoad:"MarkLoad",MarkDOMContent:"MarkDOMContent",MarkFirstPaint:"MarkFirstPaint",TimeStamp:"TimeStamp",ConsoleTime:"ConsoleTime",ResourceSendRequest:"ResourceSendRequest",ResourceReceiveResponse:"ResourceReceiveResponse",ResourceReceivedData:"ResourceReceivedData",ResourceFinish:"ResourceFinish",FunctionCall:"FunctionCall",GCEvent:"GCEvent",JSFrame:"JSFrame",UpdateCounters:"UpdateCounters",RequestAnimationFrame:"RequestAnimationFrame",CancelAnimationFrame:"CancelAnimationFrame",FireAnimationFrame:"FireAnimationFrame",WebSocketCreate:"WebSocketCreate",WebSocketSendHandshakeRequest:"WebSocketSendHandshakeRequest",WebSocketReceiveHandshakeResponse:"WebSocketReceiveHandshakeResponse",WebSocketDestroy:"WebSocketDestroy",EmbedderCallback:"EmbedderCallback",CallStack:"CallStack",SetLayerTreeId:"SetLayerTreeId",TracingStartedInPage:"TracingStartedInPage",DecodeImage:"Decode Image",ResizeImage:"Resize Image",DrawLazyPixelRef:"Draw LazyPixelRef",DecodeLazyPixelRef:"Decode LazyPixelRef",LazyPixelRef:"LazyPixelRef",LayerTreeHostImplSnapshot:"cc::LayerTreeHostImpl",PictureSnapshot:"cc::Picture"};WebInspector.TracingTimelineModel.defaultTracingCategoryFilter="*,disabled-by-default-cc.debug,disabled-by-default-devtools.timeline,disabled-by-default-devtools.timeline.frame";WebInspector.TracingTimelineModel.prototype={startRecording:function(captureStacks,captureMemory,capturePictures)
{var categories;if(WebInspector.experimentsSettings.timelineTracingMode.isEnabled()){categories=WebInspector.TracingTimelineModel.defaultTracingCategoryFilter;}else{var categoriesArray=["disabled-by-default-devtools.timeline","disabled-by-default-devtools.timeline.frame","devtools"];if(captureStacks)
categoriesArray.push("disabled-by-default-devtools.timeline.stack");if(capturePictures)
categoriesArray.push("disabled-by-default-devtools.timeline.layers","disabled-by-default-devtools.timeline.picture");categories=categoriesArray.join(",");}
this._startRecordingWithCategories(categories);},stopRecording:function()
{this._tracingModel.stop(this._didStopRecordingTraceEvents.bind(this));},setEventsForTest:function(sessionId,events)
{this.reset();this._didStartRecordingTraceEvents();this._tracingModel.setEventsForTest(sessionId,events);this._didStopRecordingTraceEvents();},_startRecordingWithCategories:function(categories)
{this.reset();this._tracingModel.start(categories,"",this._didStartRecordingTraceEvents.bind(this));},_didStartRecordingTraceEvents:function()
{this.dispatchEventToListeners(WebInspector.TimelineModel.Events.RecordingStarted);},_didStopRecordingTraceEvents:function()
{var events=this._tracingModel.devtoolsMetadataEvents();events.sort(WebInspector.TracingModel.Event.compareStartTime);this._resetProcessingState();for(var i=0,length=events.length;i<length;i++){var event=events[i];var process=event.thread.process();var startTime=event.startTime;var endTime=Infinity;if(i+1<length)
endTime=events[i+1].startTime;process.sortedThreads().forEach(this._processThreadEvents.bind(this,startTime,endTime,event.thread));}
this._resetProcessingState();this._inspectedTargetEvents.sort(WebInspector.TracingModel.Event.compareStartTime);this._buildTimelineRecords();this.dispatchEventToListeners(WebInspector.TimelineModel.Events.RecordingStopped);},minimumRecordTime:function()
...
{return this._mainThreadEvents;},reset:function()
{this._mainThreadEvents=[];this._inspectedTargetEvents=[];WebInspector.TimelineModel.prototype.reset.call(this);},_buildTimelineRecords:function()
{var recordStack=[];var mainThreadEvents=this._mainThreadEvents;for(var i=0,size=mainThreadEvents.length;i<size;++i){var event=mainThreadEvents[i];while(recordStack.length){var top=recordStack.peekLast();if(top._event.endTime>=event.startTime)
break;recordStack.pop();}
var parentRecord=recordStack.peekLast()||null;var record=new WebInspector.TracingTimelineModel.TraceEventRecord(this,event,parentRecord);if(WebInspector.TracingTimelineUIUtils.isEventDivider(record))
this._eventDividerRecords.push(record);if(!recordStack.length)
this._addTopLevelRecord(record);if(event.endTime)
recordStack.push(record);}},_addTopLevelRecord:function(record)
...
{var events=thread.events();var length=events.length;var i=events.lowerBound(startTime,function(time,event){return time-event.startTime});this._eventStack=[];for(;i<length;i++){var event=events[i];if(endTime&&event.startTime>=endTime)
break;this._processEvent(event);if(thread===mainThread)
this._mainThreadEvents.push(event);this._inspectedTargetEvents.push(event);}},_processEvent:function(event)
...
this._currentScriptEvent=null;switch(event.name){case recordTypes.CallStack:var lastMainThreadEvent=this._mainThreadEvents.peekLast();if(lastMainThreadEvent&&event.args.stack&&event.args.stack.length)
...
{return this._children;},category:function()
{var style=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(this._event.name);return style.category;},startTime:function()
{return this._event.startTime;},thread:function()
{return"CPU";},endTime:function()
{return this._event.endTime||this._event.startTime;},setEndTime:function(endTime)
...
{return this._event;}};WebInspector.TimelineFrameModelBase=function(target)
{WebInspector.TargetAwareObject.call(this,target);this.reset();}
...
{this._minimumRecordTime=Infinity;this._frames=[];this._lastFrame=null;this._lastLayerTree=null;this._hasThreadedCompositing=false;this._mainFrameCommitted=false;this._mainFrameRequested=false;this._aggregatedMainThreadWork=null;},handleBeginFrame:function(startTime)
{if(!this._lastFrame)
...
return result;}
return null;},__proto__:WebInspector.TargetAwareObject.prototype}
...
this._aggregatedMainThreadWork["other"]=(this._aggregatedMainThreadWork["other"]||0)+this._deriveOtherTime(programRecord);if(record.type()===recordTypes.CompositeLayers)
...
accounted+=programRecord.children()[i].endTime()-programRecord.children()[i].startTime();return programRecord.endTime()-programRecord.startTime()-accounted;},__proto__:WebInspector.TimelineFrameModelBase.prototype,};WebInspector.TracingTimelineFrameModel=function(target)
{WebInspector.TimelineFrameModelBase.call(this,target);}
...
{var eventNames=WebInspector.TracingTimelineModel.RecordType;if(event.phase===WebInspector.TracingModel.Phase.SnapshotObject&&event.name===eventNames.LayerTreeHostImplSnapshot&&parseInt(event.id,0)===this._layerTreeId){this.handleLayerTreeSnapshot(new WebInspector.DeferredTracingLayerTree(this.target(),event.args["snapshot"]["active_tree"]["root_layer"],event.args["snapshot"]["device_viewport_size"]));return;}
...
this._aggregatedMainThreadWork={};if(!this._aggregatedMainThreadWork)
return;if(selfTime){var categoryName=WebInspector.TracingTimelineUIUtils.eventStyle(event).category.name;this._aggregatedMainThreadWork[categoryName]=(this._aggregatedMainThreadWork[categoryName]||0)+selfTime;}
...
{if(!record.endTime())
return;var timeByCategory={};WebInspector.TimelineUIUtils.aggregateTimeForRecord(timeByCategory,record);this._addTimeForCategories(timeByCategory);},_addTimeForCategories:function(timeByCategory)
...
{this.timeByCategory[category]=(this.timeByCategory[category]||0)+time;this.cpuTime+=time;},};WebInspector.TimelineEventOverview=function(model,uiUtils)
...
return;var recordStart=Math.floor((record.startTime()-timeOffset)*scale);var recordEnd=Math.ceil((record.endTime()-timeOffset)*scale);var category=record.category();if(category.overviewStripGroupIndex<0)
...
return WebInspector.TimelineFlameChartDataProvider.jsFrameColorGenerator().colorForID(record.data()["functionName"]);return record.category().fillColorStop1;},decorateEntry:function(entryIndex,context,text,barX,barY,barWidth,barHeight,offsetToPosition)
{if(barWidth<5)
return false;var record=this._records[entryIndex];var timelineData=this._timelineData;var category=record.category();if(text){context.save();context.fillStyle="white";context.shadowColor="rgba(0, 0, 0, 0.1)";context.shadowOffsetX=1;context.shadowOffsetY=1;context.font=this._font;context.fillText(text,barX+this.textPadding(),barY+barHeight-this.textBaseline());context.restore();}
...
{WebInspector.FlameChartDataProvider.call(this);this._model=model;this._frameModel=frameModel;this._target=target;this._font="12px "+WebInspector.fontFamily();this._linkifier=new WebInspector.Linkifier();this._palette=new WebInspector.TraceViewPalette();this._entryIndexToTitle={};}
WebInspector.TracingBasedTimelineFlameChartDataProvider.prototype={barHeight:function()
{return 20;},textBaseline:function()
{return 6;},textPadding:function()
{return 5;},entryFont:function(entryIndex)
{return this._font;},entryTitle:function(entryIndex)
{var event=this._entryEvents[entryIndex];if(event){var name=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(event.name).title;var details=WebInspector.TracingTimelineUIUtils.buildDetailsNodeForTraceEvent(event,this._linkifier,false,this._target);return details?WebInspector.UIString("%s (%s)",name,details.textContent):name;}
...
{if(this._timelineData)
return this._timelineData;this._timelineData={entryLevels:[],entryTotalTimes:[],entryStartTimes:[]};this._currentLevel=0;this._minimumBoundary=this._model.minimumRecordTime();this._timeSpan=Math.max(this._model.maximumRecordTime()-this._minimumBoundary,1000);this._appendHeaderRecord("CPU");var events=this._model.mainThreadEvents();var maxStackDepth=0;for(var eventIndex=0;eventIndex<events.length;++eventIndex){var event=events[eventIndex];var category=event.category;if(category!=="disabled-by-default-devtools.timeline"&&category!=="devtools")
...
return"#555";var style=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(event.name);return style.category.fillColorStop1;},decorateEntry:function(entryIndex,context,text,barX,barY,barWidth,barHeight,offsetToPosition)
{if(barWidth<5)
...
{var event=this._entryEvents[entryIndex];if(!event)
return null;return{startTime:event.startTime,endTime:event.endTime}},paddingLeft:function()
{return 0;},textColor:function(entryIndex)
{return"white";},_appendHeaderRecord:function(title)
{var index=this._entryEvents.length;this._entryIndexToTitle[index]=title;this._entryEvents.push(null);this._timelineData.entryLevels[index]=this._currentLevel++;this._timelineData.entryTotalTimes[index]=this._timeSpan;this._timelineData.entryStartTimes[index]=this._minimumBoundary;},_appendEvent:function(event)
{var index=this._entryEvents.length;this._entryEvents.push(event);this._timelineData.entryLevels[index]=this._currentLevel+event.level;this._timelineData.entryTotalTimes[index]=event.duration||1;this._timelineData.entryStartTimes[index]=event.startTime;},createSelection:function(entryIndex)
{var event=this._entryEvents[entryIndex];if(!event)
...
{throw new Error("Not implemented.");},buildDetailsNode:function(record,linkifier,loadedFromFile)
{throw new Error("Not implemented.");},generateDetailsContent:function(record,model,linkifier,callback,loadedFromFile)
...
{return WebInspector.TimelineUIUtilsImpl.recordTitle(record);},buildDetailsNode:function(record,linkifier,loadedFromFile)
{return WebInspector.TimelineUIUtilsImpl.buildDetailsNode(record,linkifier,loadedFromFile);},generateDetailsContent:function(record,model,linkifier,callback,loadedFromFile)
{WebInspector.TimelineUIUtilsImpl.generateDetailsContent(record,model,linkifier,callback,loadedFromFile);},createBeginFrameDivider:function()
...
{var tokens=[WebInspector.TimelineUIUtilsImpl.recordTitle(record)];var data=record.data();for(var key in data)
tokens.push(data[key])
return regExp.test(tokens.join("|"));},__proto__:WebInspector.TimelineUIUtils.prototype}
WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes={};WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes[WebInspector.TimelineModel.RecordType.Layout]=1;WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes[WebInspector.TimelineModel.RecordType.Paint]=1;WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes[WebInspector.TimelineModel.RecordType.Rasterize]=1;WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes[WebInspector.TimelineModel.RecordType.DecodeImage]=1;WebInspector.TimelineUIUtilsImpl._coalescableRecordTypes[WebInspector.TimelineModel.RecordType.ResizeImage]=1;WebInspector.TimelineUIUtilsImpl.recordTitle=function(record)
...
return recordData["functionName"];if(WebInspector.TimelineUIUtilsImpl.isEventDivider(record)){var startTime=Number.millisToString(record.startTime()-record._model.minimumRecordTime());return WebInspector.UIString("%s at %s",WebInspector.TimelineUIUtils.recordStyle(record).title,startTime,true);}
...
{var imageElement=(record.getUserObject("TimelineUIUtils::preview-element")||null);var relatedNode=null;var recordData=record.data();var barrier=new CallbackBarrier();if(!imageElement&&WebInspector.TimelineUIUtilsImpl._needsPreviewElement(record.type()))
...
contentHelper.appendTextRow(WebInspector.UIString("Message"),recordData["message"]);break;case recordTypes.EmbedderCallback:contentHelper.appendTextRow(WebInspector.UIString("Callback Function"),recordData["callbackName"]);break;default:var detailsNode=WebInspector.TimelineUIUtilsImpl.buildDetailsNode(record,linkifier,loadedFromFile);if(detailsNode)
...
listRowElement.row.update(record,visibleTop,this._model.loadedFromFile(),this._uiUtils);graphRowElement.row.update(record,this._calculator,this._expandOffset,i);if(this._lastSelectedRecord===record){listRowElement.row.renderAsSelected(true);graphRowElement.row.renderAsSelected(true);}
...
return true;this._highlightedQuadRecord=record;var quad=this._uiUtils.highlightQuadForRecord(record);if(!quad)
...
WebInspector.TimelineRecordListRow.prototype={update:function(presentationRecord,offset,loadedFromFile,uiUtils)
...
this._dataElement.removeChildren();this._warningElement.classList.toggle("hidden",!presentationRecord.hasWarnings()&&!presentationRecord.childHasWarnings());this._warningElement.classList.toggle("timeline-tree-item-child-warning",presentationRecord.childHasWarnings()&&!presentationRecord.hasWarnings());if(presentationRecord.coalesced()){this._dataElement.createTextChild(WebInspector.UIString("× %d",presentationRecord.presentationChildren().length));}else{var detailsNode=uiUtils.buildDetailsNode(record,this._linkifier,loadedFromFile);if(detailsNode){this._dataElement.appendChild(document.createTextNode("("));this._dataElement.appendChild(detailsNode);this._dataElement.appendChild(document.createTextNode(")"));}}
...
{this._element.remove();}};WebInspector.TimelineTracingView=function(delegate,tracingModel,modelForMinimumBoundary)
...
{return this;},dispose:function()
{},reset:function()
{this._dataProvider.reset();this._mainView.setWindowTimes(0,Infinity);},refreshRecords:function(textFilter)
...
var contentHelper=new WebInspector.TimelineDetailsContentHelper(null,null,false);contentHelper.appendTextRow(WebInspector.UIString("Name"),record.name);contentHelper.appendTextRow(WebInspector.UIString("Category"),record.category);contentHelper.appendTextRow(WebInspector.UIString("Start"),Number.millisToString(record.startTime-this._tracingModel.minimumRecordTime(),true));contentHelper.appendTextRow(WebInspector.UIString("Duration"),Number.millisToString(record.duration,true));if(!Object.isEmpty(record.args))
...
{if(node)
contentHelper.appendElementRow("Preview",node);this._delegate.showInDetails(WebInspector.UIString("Selected Event"),contentHelper.element);}
var recordTypes=WebInspector.TracingTimelineModel.RecordType;switch(record.name){case recordTypes.PictureSnapshot:WebInspector.TracingTimelineUIUtils._buildPicturePreviewContent(record.args["snapshot"]["skp64"],appendPreviewAndshowDetails.bind(this));break;case recordTypes.LayerTreeHostImplSnapshot:var link=document.createElement("span");link.classList.add("revealable-link");link.textContent="show";link.addEventListener("click",reveal.bind(this),false);contentHelper.appendElementRow(WebInspector.UIString("Layer tree"),link);default:this._delegate.showInDetails(WebInspector.UIString("Selected Event"),contentHelper.element);}},_formatArguments:function(args)
...
return table;},__proto__:WebInspector.VBox.prototype};WebInspector.TraceViewFlameChartDataProvider=function(model,timelineModelForMinimumBoundary)
...
{var index=this._records.length;this._records.push(record);this._timelineData.entryLevels[index]=this._currentLevel++;this._timelineData.entryTotalTimes[index]=this.totalTime();this._timelineData.entryStartTimes[index]=this._minimumBoundary;this._headerTitles[index]=title;},_appendRecord:function(record,level)
{var index=this._records.length;this._records.push(record);this._timelineData.entryLevels[index]=this._currentLevel+level;this._timelineData.entryTotalTimes[index]=record.phase===WebInspector.TracingModel.Phase.SnapshotObject?NaN:record.duration||0;this._timelineData.entryStartTimes[index]=record.startTime;},_isHeaderRecord:function(record)
...
this._pendingStopCallback=callback;TracingAgent.end();},sessionId:function()
{return this._sessionId;},setEventsForTest:function(sessionId,events)
{this.reset();this._sessionId=sessionId;this._eventsCollected(events);this._tracingComplete();},_bufferUsage:function(usage)
...
{this._active=false;if(!this._pendingStopCallback)
return;this._pendingStopCallback();this._pendingStopCallback=null;},reset:function()
{this._processById={};this._minimumRecordTime=0;this._maximumRecordTime=0;this._sessionId=null;this._devtoolsMetadataEvents=[];},_addEvent:function(payload)
...
var thread=process.threadById(payload.tid);if(payload.ph!==WebInspector.TracingModel.Phase.Metadata){var timestamp=payload.ts/1000;if(timestamp&&(!this._minimumRecordTime||timestamp<this._minimumRecordTime))
this._minimumRecordTime=timestamp;if(!this._maximumRecordTime||timestamp>this._maximumRecordTime)
this._maximumRecordTime=timestamp;var event=thread.addEvent(payload);if(payload.ph===WebInspector.TracingModel.Phase.SnapshotObject)
process.addObject(event);if(event&&event.name===WebInspector.TracingModel.DevToolsMetadataEvent.TracingStartedInPage&&event.category===WebInspector.TracingModel.DevToolsMetadataEventCategory&&event.args["sessionId"]===this._sessionId)
this._devtoolsMetadataEvents.push(event);return;}
switch(payload.name){case WebInspector.TracingModel.MetadataEvent.ProcessSortIndex:process._setSortIndex(payload.args["sort_index"]);break;case WebInspector.TracingModel.MetadataEvent.ProcessName:process._setName(payload.args["name"]);break;case WebInspector.TracingModel.MetadataEvent.ThreadSortIndex:thread._setSortIndex(payload.args["sort_index"]);break;case WebInspector.TracingModel.MetadataEvent.ThreadName:thread._setName(payload.args["name"]);break;}},minimumRecordTime:function()
{return this._minimumRecordTime;},maximumRecordTime:function()
...
{this.name=payload.name;this.category=payload.cat;this.startTime=payload.ts/1000;this.args=payload.args;this.phase=payload.ph;this.level=level;if(payload.dur)
this._setEndTime((payload.ts+payload.dur)/1000);if(payload.id)
...
WebInspector.TracingModel.Event.prototype={_setEndTime:function(endTime)
{if(endTime<this.startTime){console.assert(false,"Event out of order: "+this.name);return;}
this.endTime=endTime;this.duration=endTime-this.startTime;},_complete:function(payload)
...
{WebInspector.TracingModel.NamedObject.call(this);this._process=process;this._setName("Thread "+id);this._events=[];this._stack=[];this._maxStackDepth=0;}
WebInspector.TracingModel.Thread.prototype={addEvent:function(payload)
{for(var top=this._stack.peekLast();top&&top.endTime&&top.endTime<=payload.ts/1000;){this._stack.pop();top=this._stack.peekLast();}
...
var event=new WebInspector.TracingModel.Event(payload,this._stack.length,this);if(payload.ph===WebInspector.TracingModel.Phase.Begin||payload.ph===WebInspector.TracingModel.Phase.Complete){this._stack.push(event);if(this._maxStackDepth<this._stack.length)
this._maxStackDepth=this._stack.length;}
if(this._events.length&&this._events.peekLast().startTime>event.startTime)
console.assert(false,"Event is our of order: "+event.name);this._events.push(event);return event;},process:function()
...
{this._tracingModel._tracingComplete();}};WebInspector.TracingTimelineUIUtils=function()
{WebInspector.TimelineUIUtils.call(this);}
...
{return WebInspector.TracingTimelineUIUtils.isEventDivider(record);},countersForRecord:function(record)
...
{return WebInspector.TracingTimelineUIUtils.styleForTraceEvent(record.traceEvent().name).title;},buildDetailsNode:function(record,linkifier,loadedFromFile)
{return WebInspector.TracingTimelineUIUtils.buildDetailsNodeForTraceEvent(record.traceEvent(),linkifier,loadedFromFile,record.target());},generateDetailsContent:function(record,model,linkifier,callback,loadedFromFile)
{if(!(model instanceof WebInspector.TracingTimelineModel))
throw new Error("Illegal argument.");var tracingTimelineModel=(model);WebInspector.TracingTimelineUIUtils.buildTraceEventDetails(record.traceEvent(),tracingTimelineModel,linkifier,callback,loadedFromFile,record.target());},createBeginFrameDivider:function()
...
{var traceEvent=record.traceEvent();var title=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(traceEvent.name).title;var tokens=[title];for(var argName in traceEvent.args){var argValue=traceEvent.args[argName];for(var key in argValue)
...
return WebInspector.TracingTimelineUIUtils._eventStylesMap;var recordTypes=WebInspector.TracingTimelineModel.RecordType;var categories=WebInspector.TimelineUIUtils.categories();var eventStyles={};eventStyles[recordTypes.Program]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Other"),categories["other"]);eventStyles[recordTypes.EventDispatch]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Event"),categories["scripting"]);eventStyles[recordTypes.RequestMainThreadFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Request Main Thread Frame"),categories["rendering"]);eventStyles[recordTypes.BeginFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Frame Start"),categories["rendering"]);eventStyles[recordTypes.BeginMainThreadFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Frame Start (main thread)"),categories["rendering"]);eventStyles[recordTypes.DrawFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Draw Frame"),categories["rendering"]);eventStyles[recordTypes.ScheduleStyleRecalculation]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Schedule Style Recalculation"),categories["rendering"]);eventStyles[recordTypes.RecalculateStyles]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Recalculate Style"),categories["rendering"]);eventStyles[recordTypes.InvalidateLayout]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Invalidate Layout"),categories["rendering"]);eventStyles[recordTypes.Layout]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Layout"),categories["rendering"]);eventStyles[recordTypes.PaintSetup]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Paint Setup"),categories["painting"]);eventStyles[recordTypes.UpdateLayer]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Update Layer"),categories["painting"]);eventStyles[recordTypes.Paint]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Paint"),categories["painting"]);eventStyles[recordTypes.Rasterize]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Paint"),categories["painting"]);eventStyles[recordTypes.RasterTask]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Paint"),categories["painting"]);eventStyles[recordTypes.ScrollLayer]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Scroll"),categories["rendering"]);eventStyles[recordTypes.CompositeLayers]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Composite Layers"),categories["painting"]);eventStyles[recordTypes.ParseHTML]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Parse HTML"),categories["loading"]);eventStyles[recordTypes.TimerInstall]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Install Timer"),categories["scripting"]);eventStyles[recordTypes.TimerRemove]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Remove Timer"),categories["scripting"]);eventStyles[recordTypes.TimerFire]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Timer Fired"),categories["scripting"]);eventStyles[recordTypes.XHRReadyStateChange]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("XHR Ready State Change"),categories["scripting"]);eventStyles[recordTypes.XHRLoad]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("XHR Load"),categories["scripting"]);eventStyles[recordTypes.EvaluateScript]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Evaluate Script"),categories["scripting"]);eventStyles[recordTypes.MarkLoad]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Load event"),categories["scripting"]);eventStyles[recordTypes.MarkDOMContent]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("DOMContentLoaded event"),categories["scripting"]);eventStyles[recordTypes.MarkFirstPaint]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("First paint"),categories["painting"]);eventStyles[recordTypes.TimeStamp]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Stamp"),categories["scripting"]);eventStyles[recordTypes.ConsoleTime]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Console Time"),categories["scripting"]);eventStyles[recordTypes.ResourceSendRequest]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Send Request"),categories["loading"]);eventStyles[recordTypes.ResourceReceiveResponse]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Receive Response"),categories["loading"]);eventStyles[recordTypes.ResourceFinish]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Finish Loading"),categories["loading"]);eventStyles[recordTypes.ResourceReceivedData]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Receive Data"),categories["loading"]);eventStyles[recordTypes.FunctionCall]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Function Call"),categories["scripting"]);eventStyles[recordTypes.GCEvent]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("GC Event"),categories["scripting"]);eventStyles[recordTypes.JSFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("JS Frame"),categories["scripting"]);eventStyles[recordTypes.RequestAnimationFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Request Animation Frame"),categories["scripting"]);eventStyles[recordTypes.CancelAnimationFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Cancel Animation Frame"),categories["scripting"]);eventStyles[recordTypes.FireAnimationFrame]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Animation Frame Fired"),categories["scripting"]);eventStyles[recordTypes.WebSocketCreate]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Create WebSocket"),categories["scripting"]);eventStyles[recordTypes.WebSocketSendHandshakeRequest]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Send WebSocket Handshake"),categories["scripting"]);eventStyles[recordTypes.WebSocketReceiveHandshakeResponse]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Receive WebSocket Handshake"),categories["scripting"]);eventStyles[recordTypes.WebSocketDestroy]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Destroy WebSocket"),categories["scripting"]);eventStyles[recordTypes.EmbedderCallback]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Embedder Callback"),categories["scripting"]);eventStyles[recordTypes.DecodeImage]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Image Decode"),categories["painting"]);eventStyles[recordTypes.ResizeImage]=new WebInspector.TimelineRecordStyle(WebInspector.UIString("Image Resize"),categories["painting"]);WebInspector.TracingTimelineUIUtils._eventStylesMap=eventStyles;return eventStyles;}
WebInspector.TracingTimelineUIUtils._coalescableRecordTypes={};WebInspector.TracingTimelineUIUtils._coalescableRecordTypes[WebInspector.TracingTimelineModel.RecordType.Layout]=1;WebInspector.TracingTimelineUIUtils._coalescableRecordTypes[WebInspector.TracingTimelineModel.RecordType.Paint]=1;WebInspector.TracingTimelineUIUtils._coalescableRecordTypes[WebInspector.TracingTimelineModel.RecordType.Rasterize]=1;WebInspector.TracingTimelineUIUtils._coalescableRecordTypes[WebInspector.TracingTimelineModel.RecordType.DecodeImage]=1;WebInspector.TracingTimelineUIUtils._coalescableRecordTypes[WebInspector.TracingTimelineModel.RecordType.ResizeImage]=1;WebInspector.TracingTimelineUIUtils.eventStyle=function(event)
...
{var relatedNode=null;var barrier=new CallbackBarrier();if(!event.previewElement){if(event.imageURL)
...
contentHelper.appendTextRow(WebInspector.UIString("Message"),eventData["message"]);break;case recordTypes.EmbedderCallback:contentHelper.appendTextRow(WebInspector.UIString("Callback Function"),eventData["callbackName"]);break;default:var detailsNode=WebInspector.TracingTimelineUIUtils.buildDetailsNodeForTraceEvent(event,linkifier,loadedFromFile,target);if(detailsNode)
contentHelper.appendElementRow(WebInspector.UIString("Details"),detailsNode);break;}
if(relatedNode)
...
continue;if(i>index)
hasChildren=true;var category=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(nextEvent.name).category.name;aggregatedStats[category]=(aggregatedStats[category]||0)+nextEvent.selfTime;}}
...
{if(error){console.error("LayerTreeAgent.loadSnapshot(): "+error);callback();return;}
snapshotId=id;LayerTreeAgent.replaySnapshot(snapshotId,onSnapshotReplayed);}
function onSnapshotReplayed(error,encodedBitmap)
{LayerTreeAgent.releaseSnapshot(snapshotId);if(error){console.error("LayerTreeAgent.replaySnapshot(): "+error);callback();return;}
var container=document.createElement("div");container.className="image-preview-container";var img=container.createChild("img");img.src=encodedBitmap;callback(container);}}
...
eventDivider.className+=" resources-orange-divider";else if(recordType===recordTypes.BeginFrame)
eventDivider.className+=" timeline-frame-divider";if(title)
eventDivider.title=title;return eventDivider;};WebInspector.TransformController=function(element)
...
return this._lazyFrameModel;if(this._tracingModel){var tracingFrameModel=new WebInspector.TracingTimelineFrameModel(this._model.target());tracingFrameModel.addTraceEvents(this._tracingTimelineModel.inspectedTargetEvents(),this._tracingModel.sessionId()||"");this._lazyFrameModel=tracingFrameModel;}else{var frameModel=new WebInspector.TimelineFrameModel(this._model.target());frameModel.setMergeRecords(!WebInspector.experimentsSettings.timelineNoLiveUpdate.isEnabled()||!this._recordingInProgress);frameModel.addRecords(this._model.records());this._lazyFrameModel=frameModel;}
...
if(WebInspector.experimentsSettings.timelineTracingMode.isEnabled()){this._captureTracingSetting=WebInspector.settings.createSetting("timelineCaptureTracing",false);topPaneSidebarElement.appendChild(WebInspector.SettingsUI.createSettingCheckbox(WebInspector.UIString("Capture tracing"),this._captureTracingSetting,true,undefined,WebInspector.UIString("Capture tracing information")));this._captureTracingSetting.addChangeListener(this._onModeChanged,this);}else if(WebInspector.experimentsSettings.timelineOnTraceEvents.isEnabled()){this._captureLayersAndPicturesSetting=WebInspector.settings.createSetting("timelineCaptureLayersAndPictures",false);topPaneSidebarElement.appendChild(WebInspector.SettingsUI.createSettingCheckbox(WebInspector.UIString("Capture pictures"),this._captureLayersAndPicturesSetting,true,undefined,WebInspector.UIString("Capture graphics layer positions and painted pictures")));}},_createStatusBarItems:function()
...
this._overviewControls.push(new WebInspector.TimelineEventOverview(this._model,this._uiUtils));if(WebInspector.experimentsSettings.timelineFlameChart.isEnabled()&&this._flameChartEnabledSetting.get()){var tracingTimelineModel=WebInspector.experimentsSettings.timelineOnTraceEvents.isEnabled()?this._tracingTimelineModel:null;this._addModeView(new WebInspector.TimelineFlameChart(this,this._model,tracingTimelineModel,this._frameModel()));}else{this._addModeView(this._timelineView());}
...
{this._userInitiatedRecording=userInitiated;this._model.startRecording(this._captureStacksSetting.get(),this._captureMemorySetting.get(),this._captureLayersAndPicturesSetting&&this._captureLayersAndPicturesSetting.get());if(WebInspector.experimentsSettings.timelineNoLiveUpdate.isEnabled()&&this._lazyFrameModel)
...
{this._stopPending=false;this._updateToggleTimelineButton(false);if(this._lazyFrameModel){if(this._tracingTimelineModel){this._lazyFrameModel.reset();this._lazyFrameModel.addTraceEvents(this._tracingTimelineModel.inspectedTargetEvents(),this._tracingModel.sessionId());this._overviewPane.update();}else if(WebInspector.experimentsSettings.timelineNoLiveUpdate.isEnabled()){this._lazyFrameModel.reset();this._lazyFrameModel.addRecords(this._model.records());}}
...
this._hideProgressPane();},_onRecordAdded:function(event)
...
switch(this._selection.type()){case WebInspector.TimelineSelection.Type.Record:var record=(this._selection.object());var title=this._uiUtils.titleForRecord(record);this._uiUtils.generateDetailsContent(record,this._model,this._detailsLinkifier,this.showInDetails.bind(this,title),this._model.loadedFromFile());break;case WebInspector.TimelineSelection.Type.TraceEvent:var event=(this._selection.object());var title=WebInspector.TracingTimelineUIUtils.styleForTraceEvent(event.name).title;WebInspector.TracingTimelineUIUtils.buildTraceEventDetails(event,this._tracingTimelineModel,this._detailsLinkifier,this.showInDetails.bind(this,title),false,this._model.target());break;case WebInspector.TimelineSelection.Type.Frame:var frame=(this._selection.object());this.showInDetails(WebInspector.UIString("Frame Statistics"),WebInspector.TimelineUIUtils.generateDetailsContentForFrame(this._lazyFrameModel,frame));if(frame.layerTree){var layersView=this._layersView();layersView.showLayerTree(frame.layerTree);this._detailsView.appendTab("layers",WebInspector.UIString("Layers"),layersView);}
...
var categoryName=record.category().name;var ownTime=Math.min(endTime,record.endTime())-Math.max(startTime,record.startTime())-childrenTime;aggregatedStats[categoryName]=(aggregatedStats[categoryName]||0)+ownTime;}
...
{WebInspector.TabbedPane.call(this);this._defaultDetailsView=new WebInspector.VBox();this._defaultDetailsView.element.classList.add("timeline-details-view");this._defaultDetailsContentElement=this._defaultDetailsView.element.createChild("div","timeline-details-view-body");this.appendTab("default",WebInspector.UIString("Details"),this._defaultDetailsView);this.addEventListener(WebInspector.TabbedPane.EventTypes.TabSelected,this._tabSelected,this);}
WebInspector.TimelineDetailsView.prototype={setContent:function(title,node)
{this.changeTabTitle("default",WebInspector.UIString("Details: %s",title));var otherTabs=this.otherTabs("default");for(var i=0;i<otherTabs.length;++i)
...
{this._defaultDetailsContentElement.classList.toggle("hbox",!vertical);this._defaultDetailsContentElement.classList.toggle("vbox",vertical);},appendTab:function(id,tabTitle,view,tabTooltip)
{WebInspector.TabbedPane.prototype.appendTab.call(this,id,tabTitle,view,tabTooltip);if(this._lastUserSelectedTabId===id)
