'use strict';var _exports;function init(){function a(I,J){let K=JSON.parse(decodeURIComponent(J)),L=K.file,M=K.msg.codeFrame;I.executeScript({code:`console.group("${new Date} js 文件，编译错误")
        console.error(\`${L}\n${M}\`)
      console.groupEnd()`})}function b(I,J){let K=J.target.src,L=r.parse(K),M=L.pathname.replace(/html$/,'wxml'),N=H[J.level],O=(J.message||'').replace(A,'').replace(/\"/g,'\\"');I.executeScript({code:`console.group("${new Date} ${M}")
        console.${N}("${O}")
      console.groupEnd()`})}function c(I,J){let K=JSON.parse(decodeURIComponent(J.replace(C,''))),L=K.file,M=K.msg.join('\n').replace(/\`/g,'\'');I.executeScript({code:`console.group("${new Date} wxml 文件，编译错误")
        console.error(\`${L}\n${M}\`)
